...
14	What kinds of computing devices do you own or frequently use? (Select all that apply)	select	multiple	\N	1
...
COPY user_responses (value, comment, user_id, question_id, response_id, "timestamp") FROM stdin;
\.
...
COPY user_traces (id, tile_id, "timestamp", query, user_id, dataset_id) FROM stdin;
\.
...
COPY users (id, flask_session_id) FROM stdin;
\.
...
ALTER TABLE ONLY user_responses
    ADD CONSTRAINT user_responses_pkey PRIMARY KEY (user_id, question_id);
